...
namespace Tugas4
{
    public partial class Form1 : Form
    {
        ...
        private void Button_cariPengarang_Click(object sender, EventArgs e)
        {
            pengarang = true;
            judul = false;
            cari_buku();
        ...
        private void Button_cariJudul_Click(object sender, EventArgs e)
        {
            pengarang = false;
            judul = true;
            cari_buku();
        ...
        private void cari_buku()
        {
            listBuku.Items.Clear(); try
            {
                ...
                databaseConnection.Open();
                MySqlCommand cmd = new MySqlCommand(query, databaseConnection);
                cmd.Parameters.AddWithValue("@dicari", textBox_Cari.Text);
                cmd.CommandTimeout = 60;
                MySqlDataReader reader = cmd.ExecuteReader(); if (reader.HasRows)
                {
                    while (reader.Read())
                    {
                        ListViewItem listViewItem = new ListViewItem(reader["id_buku"].ToString());
